...
**Post-Task-6 correction:** Task 6's code review (and two follow-up re-reviews) found that Task 5's renderer changes, while spec-correct in isolation, didn't account for `SortableTable`'s pre-existing DOM-based re-sort logic in `js/tables.js` (code that predates this redesign, not written by Task 5 or Task 6). Fixed across three follow-up commits on `redesign/crimson-gold` (205f407, 6e7c90c, d633a91): `sortBy()` was clobbering movement-arrow HTML with a bare rank number on every render including the initial one, never recomputing rank-tier row classes at all, matching rows to their data by positional index in a way that went stale after a second sort, and — caught last, by actually testing ascending sort rather than just repeated descending sorts — inverting the rank-tier bar's color whenever a table was sorted ascending. A regression test (`tests/sortableTable.test.js`) now covers this. No action needed for future tasks; noted here for the record.
